...
class TestModel(unittest.TestCase):
    ...
    def test_forward(self, model, inputs, use_pf, fp16):
        ...
        print("forward:", output.logits_per_image)
        torch.save(
            output.logits_per_image,
            f"output-{'pf' if use_pf else 'non-pf'}-{'fp16' if fp16 else 'fp32'}.pt",
        )
        assert isinstance(output.logits_per_image, torch.Tensor)
...
if __name__ == "__main__":
    ...
    processor = CLIPProcessor.from_pretrained(args.name)
    model = CLIPModel.from_pretrained(args.name).eval()
    inputs = processor(
