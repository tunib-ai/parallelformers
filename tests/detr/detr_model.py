...
class TestModel(unittest.TestCase):
    ...
    def test_forward(self, model, inputs, use_pf, fp16):
        ...
        print("forward:", output.last_hidden_state[0][0])
        torch.save(
            output.last_hidden_state,
            f"output-{'pf' if use_pf else 'non-pf'}-{'fp16' if fp16 else 'fp32'}.pt",
        )
        assert isinstance(output.last_hidden_state[0][0], torch.Tensor)
...
if __name__ == "__main__":
    ...
    feature_extractor = DetrFeatureExtractor.from_pretrained(args.name)
    model = DetrModel.from_pretrained(args.name).eval()
